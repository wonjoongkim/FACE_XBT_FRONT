...
export const WrongAnswerC = (props) => {
    ...
    const [breakModalOpen, setBreakModalOpen] = useState(false); // 학습중 쉬는 시간 Modal창
    ...
    const [cut_time, setCut_time] = useState(10000);
    ...
    const WrongAnswer_ApiCall = async () => {
        ...
        if (WrongAnswerResponse?.data?.RET_CODE !== '0000') {
            setLoading(false);
            setDeadlineModalOpen(true);
        }
        console.log(WrongAnswerResponse?.data?.RET_DATA);
        setWrongAnswerData(WrongAnswerResponse?.data?.RET_DATA);
        setImageTotal(WrongAnswerResponse?.data?.RET_DATA?.learningProblemList?.length); // 총 문항 수
    ...
    const learn02_start = () => {
        ...
        function startTimer() {
            intervalId = setInterval(() => {
                setState((prevState) => {
                    ...
                    setSaveTime({ ...saveTime, minutes: minutes, seconds: seconds });
        ...
        $('#learn03_start_break').show();
        $('#learn03_start_break_on').hide();
        ...
        // (쉬는 시간) 타이머가 진행될 때 멈춤
        $('#learn03_start_break').click(function () {
            $('#learn03_start_break').hide();
            $('#learn03_start_break_on').show();
            setIsBlur(true);
            if (status === 0) {
                pause();
                status = 1;
        ...
        // (학습 시작) 멈춘 타이머 이어서 시작
        $('#learn03_start_break_on').click(function () {
            $('#learn03_start_break').show();
            $('#learn03_start_break_on').hide();
            setIsBlur(false);
            startTimer();
        ...
        $('.learnbtc04 button').click(async function () {
            if ($(this)[0].id == 'learn02_Open') {
                ...
                if ($(this)[0].id == 'learn02_Pass') {
                    UpdateWrongAnswer_ApiCall('4', $(images[currentImageIndex]).data('value')); // 미개봉/통과 pass [4]
                } else if ($(this)[0].id == 'learn02_Prohibited') {
                    UpdateWrongAnswer_ApiCall('1', $(images[currentImageIndex]).data('value')); // 미개봉/금지 Prohibited [1]
                } else if ($(this)[0].id == 'learn02_Open_Pass') {
                    UpdateWrongAnswer_ApiCall('3', $(images[currentImageIndex]).data('value')); // 개봉/제한 Open/Pass [3]
                } else if ($(this)[0].id == 'learn02_Open_Resricted') {
                    UpdateWrongAnswer_ApiCall('2', $(images[currentImageIndex]).data('value')); // 개봉/통과 Open/Resricted [2]
                } else if ($(this)[0].id == 'learn02_Open_Prohibited') {
                    UpdateWrongAnswer_ApiCall('0', $(images[currentImageIndex]).data('value')); // 개봉/금지 Open/Prohibited [0]
                }
                learn02_btn();
                setAnswerType(null);
            }
    ...
    return (
        <>
            <Spin spinning={loading}>
                <div className="learn_con">
                    ...
                    <div className="xbt_top">
                        ...
                        <div className="learnct01">
                            <ul>
                                <li>
                                    <h1 className="contit">{languageApplyInfoData.oxplay1}(Cut)</h1>
                                </li>
                                <li>
                            ...
                            <ul>
                                ...
                                <li>
                                    <button className="learnbtn btn_start" id="learn02_start" type="button" onClick={learn02_start}>
                                        {languageApplyInfoData.oxplay3}
                                    </button>
                                    {/* <button className="learnbtn btn_end" id="learn02_start_on" type="button">
                                        {languageApplyInfoData.oxplay3}
                                    </button> */}
                                    <button className="learnbtn btn_break" id="learn03_start_break" type="button">
                                        쉬는 시간
                                    </button>
                                    <button className="learnbtn btn_break_on" id="learn03_start_break_on" type="button">
                                        학습 시작
                                    </button>
                                    <button
                    ...
                    <div className="learnc_img" id="learn02_img">
                        <div id="container" className={isBlur ? 'blur' : ''}>
                            <div id="learn02_progress"></div>
                            {wrongAnswerData?.learningProblemList?.map((imgs, i) => {
                                return (
                                    <>
                                        <img
                                            key={i}
                ...
                {/* <!-- learn_bottom --> */}
                <div className={isBlur ? 'learn_bottom blur' : 'learn_bottom'}>
                    {/* <!-- learn_btcon --> */}
                        ...
                        <div className="learnbtc03">
                            <ul>
                                ...
                                <li>
                                    ...
                                    >
                                        <img src={transform} alt="좌우반전" title="Transfrom" />
                                    </a>
                                </li>
                                {/* 축소 */}
                                <li>
            ...
            <Modal
                ...
                footer={null}
            >
                <WrongAnswerP ModalClose2={ModalClose2} MenuCd={props.MenuCd} />
            </Modal>
            {/* 정답 확인 End */}
            ...
            >
                <div style={{ width: '542px', textAlign: 'center', padding: '50px 0px' }}>
                    <div className="scwd_txt01">
                        <h1>{languageApplyInfoData.oxplay6}</h1>
                    </div>
                    <div className="scwd_txt02">
                        <p>{languageApplyInfoData.oxplay7}</p>
                    </div>
                    <button
                        id="open-six-modal"
                        data-mact="open"
            ...
            >
                <div style={{ width: '542px', textAlign: 'center', padding: '50px 0px' }}>
                    ...
                    <div className="scwd_txt01">
                        <h3>
                            {wrongAnswerData?.moduleNm}{' '}
                            <span>
                                {languageApplyInfoData.oxplay9} : {endWrongAnswerData?.gainScore}
                            </span>
                        </h3>
                    </div>
                    <div className="scwd_txt02">
                        <h1>
                            <span className="scwd_pass">{languageApplyInfoData.oxplay12}</span>
                        </h1>
                    </div>
                    <button
                        id="open-sev-modal"
            ...
            >
                <div style={{ width: '542px', textAlign: 'center', padding: '50px 0px' }}>
                    ...
                    <div className="scwd_txt02">
                        <h1>
                            <span className="scwd_fail">{languageApplyInfoData.oxplay11}</span>
                        </h1>
                    </div>
                    <button
                        id="open-eig-modal"
            ...
            <Modal
                ...
                footer={[]}
            >
                <div style={{ textAlign: 'center', padding: '119px' }}>
                    <div>
                        <img src={fail_color} alt="" />
                    </div>
                    <div className="scwd_txt02">
                        <h1>
                            <span className="scwd_fail">오답문제풀이가 잠시 중지된 상태입니다...</span>
                        </h1>
                    </div>
                    <button
                        id="open-eig-modal"
