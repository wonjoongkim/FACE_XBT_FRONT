...
export const LearningS = (props) => {
    ...
    const Learning_ApiCall = async () => {
        ...
        if (LearningResponse?.data?.RET_CODE !== '0000') {
            setLoading(false);
    ...
    const learn01_start = () => {
        ...
        let move_timer = learningData.slideSpeed; //움직이는 시간(ms, 1/1000초) (slide_time / learningData.timeLimit)
        let move_distance = learningData.slideSpeed; //움직일 거리 (slide_speed / learningData.slideSpeed)
        // var move_timer = 5; //움직이는 시간(ms, 1/1000초) (slide_time / learningData.timeLimit)
        // var move_distance = 10; //움직일 거리 (slide_speed / learningData.slideSpeed)
        let is_learn01_play = true;
        let intervalId;
        startTimer();
        ...
        //<====================
        // Pass, Open, Prohibited, Resricted 버튼 눌렀을 때 처리할 부분
        // 다음 이미지로 넘어감
        ...
        $('.learnbtc04 button').click(async function () {
            if ($(this)[0].id == 'learn01_Open') {
                ...
                if ($(this)[0].id == 'learn01_Pass') {
                    UpdateLearnAnswer_ApiCall('4', bag_id[current_image]); // 미개봉/통과 pass [4]
                } else if ($(this)[0].id == 'learn01_Prohibited') {
                    UpdateLearnAnswer_ApiCall('1', bag_id[current_image]); // 미개봉/금지 Prohibited [1]
                } else if ($(this)[0].id == 'learn01_Open_Pass') {
                    UpdateLearnAnswer_ApiCall('3', bag_id[current_image]); // 개봉/제한 Open/Pass [3]
                } else if ($(this)[0].id == 'learn01_Open_Resricted') {
                    UpdateLearnAnswer_ApiCall('2', bag_id[current_image]); // 개봉/통과 Open/Resricted [2]
                } else if ($(this)[0].id == 'learn01_Open_Prohibited') {
                    UpdateLearnAnswer_ApiCall('0', bag_id[current_image]); // 개봉/금지 Open/Prohibited [0]
                }
                learn01_btn();
                setAnswerType(null);
            }
    ...
    return (
        <>
                ...
                <div className="learn_con">
                    <div className="xbt_top">
                        <div className="learnct01">
                            <ul>
                                <li>
                                    <h1 className="contit">{languageApplyInfoData.learning1}(Slide)</h1>
                                </li>
                                <li>
            ...
            <Modal
                ...
                footer={null}
            >
                <LearningP ModalClose2={ModalClose2} MenuCd={props.MenuCd} />
            </Modal>
            {/* 정답 확인 End */}
                ...
                <div style={{ width: '542px', textAlign: 'center', padding: '50px 0px' }}>
                    ...
                    <div className="scwd_txt01">
                        <h3>
                            ...
                            <span>
                                {languageApplyInfoData.learning9} : {endLearningData?.gainScore}
                            </span>
                        </h3>
                    </div>
                    <div className="scwd_txt02">
            ...
            >
                <div style={{ textAlign: 'center', padding: '119px' }}>
                    ...
                    <div className="scwd_txt02">
                        <h1>
                            <span className="scwd_fail">학습이 잠시 중지된 상태입니다...</span>
                        </h1>
                    </div>
                    <button
                        id="open-eig-modal"
